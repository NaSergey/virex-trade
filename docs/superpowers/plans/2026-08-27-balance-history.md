...
- [ ] **Step 7: Перенести стоп в сделку тем же проходом, что и `openedAt`**

Вопреки первоначальной формулировке плана, `openedAt` **не** подставляется при вставке сделки: его дозаполняет отдельный проход `fillOpenedAt` в `trade-sync.service.ts` (~строки 303-321) через `updateMany` по всем строкам реестра. Стоп едет туда же — это тот же самый стамп из той же самой строки, и разносить их по разным механизмам значило бы завести второй проход с собственными условиями и собственными расхождениями.

Переименовать метод в `fillEntryStamps` (он больше не про одно поле) и расширить `data`:
...
Обновить единственное место вызова (искать `fillOpenedAt` грепом по `backend/src/`). Сделки, уже помеченные `openedAt` до этой правки, стопа не получат никогда — их строка реестра давно удалена вместе с закрытой позицией. Это ожидаемо и совпадает с тем, что записано в спеке: `planned_risk_pct` работает только вперёд.
...
План выполнен целиком; ниже — то, что ревью сочло несущественным для слияния, но что стоит
помнить второму плану.

- **`TradeRiskService.computeMissing` без пакетного лимита.** Соседний `TradeContextService`
  обрабатывает сделки порциями с `BATCH_LIMIT` и самозалечивающимся проходом по тикам, здесь же
  цикл идёт по всем недостающим сразу, по три запроса к базе на сделку. На первом прогоне
  пользователя с полугодовой историей это будет заметно медленным. Не баг, но расхождение с уже
  принятым в кодовой базе приёмом.
- **Нет теста на вывод баланса ПОСЛЕ последнего якоря.** Ветка продления вперёд в `segmentFor`
  использует тот же код, что и продление назад, и покрыта косвенно. Но `balance-chain.ts` назван
  единственной формулой подсистемы, и дыру в его покрытии стоит закрыть в ближайшую итерацию, а
  не откладывать бессрочно.
- **`sumFlows` не оговаривает случай `from > to`.** Оба реальных вызывающих места гарантируют
  `from <= to`, поэтому сейчас это безопасно. Стоит дописать предостережение в комментарий при
  следующей правке файла.
- **Лог `stamped openedAt on N trade(s)`** не упоминает, что тем же проходом ставится и стоп.
- **Выводы о поведении API OKX, KuCoin и MEXC** собраны из косвенных источников, а не из прямой
  документации, и помечены в спеке как выводы. Перепроверить на живых данных — но живых сделок по
  этим биржам у продукта пока и нет.
- **У Binance `openFee` всегда 0:** комиссия входа приходит на отдельном филле и в строку сделки
  не попадает, поэтому цепочка систематически её недосчитывает. Принято сознательно и терпимо
  ровно при часовых якорях и допуске 0.5%. При переходе на более редкие якоря — пересмотреть.
