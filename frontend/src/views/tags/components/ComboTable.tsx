  ...
  return (
    ...
    <div data-tour="tags-combos">
      <LedgerTable
        ...
        rowKey={(r) => r.key}
        /*
         * Клик по строке — дрилдаун в Аналитику с теми же тегами в условиях:
         * вопрос «а что это были за сделки» возникает ровно здесь, у сводного
         * числа, и раньше на него отвечали, перещёлкивая те же теги руками в
         * другом разделе. Имя `tags` в query читает useLabFilters — тот же
         * вход, которым работают привычки с Обзора (`habitSearchParams`).
         *
         * Строка без тегов («без тегов» — отдельный бакет) никуда не ведёт:
         * пустой набор в Аналитике означал бы «все сделки», а не эту строку.
         */
        rowHref={(r) =>
          r.combo.tagIds.length ? `/analytics?tags=${r.combo.tagIds.join(',')}` : undefined
        }
        minWidth={980}
        isLoading={isLoading}
