...
describe('buildEquityGeometry', () => {
  ...
  it('находит яму под сделкой и молчит там, где кривая на рекорде', () => {
    // Яма 100 → 40 с возвратом на i=5: под водой только i=2..4.
    const g = buildEquityGeometry(points(10, 100, 70, 40, 80, 100, 130), 300)!;
    expect(dipAt(g, 3)?.depth).toBe(60);
    expect(dipAt(g, 2)?.fromIdx).toBe(1);
    expect(dipAt(g, 1)).toBeNull(); // вершина, с которой начали падать
    expect(dipAt(g, 5)).toBeNull(); // возврат к рекорду
    expect(dipAt(g, 6)).toBeNull(); // новый максимум
  });

  it('неотыгранная яма держится до последней сделки включительно', () => {
    const g = buildEquityGeometry(points(10, 100, 60, 70), 300)!;
    expect(dipAt(g, 3)?.recovered).toBe(false);
  });
