...
export const NOTIF_DEFS: NotifDef[] = [
  ...
  },
  {
    ...
    // value — во сколько раз этот день недели волатильнее среднего дня в тот
    // же час. Порог не по самой волатильности: час сравнивается сам с собой в
    // другие дни, и «×1.2» здесь значит «в этот день он заметно живее, чем в
    // остальные». Пресет 0 старой версии («только час») попадает на ×1.1,
    // пресет 1 («час + слабый день») — на новый дефолт; строка про слабый для
    // лонга день теперь идёт в сообщении всегда, отдельным пресетом её больше
    // не выбирают — сигнал и так стал раз в сутки.
    presets: [
      { value: 1.1, label: '×1.1' },
      { value: 1.2, label: '×1.2' },
    ...
    defaultPreset: 1,
    defaultEnabled: false,
    // Кандидат в сутках ровно один (peakHourOfWeekday), поэтому cooldown здесь
    // не ограничитель частоты, а страховка от стыка суток UTC: пик воскресенья
    // в 23:00 и пик понедельника в 00:00 — это два сообщения подряд.
    cooldownMs: 6 * HOUR,
    ignoresQuietHours: false,
  },
  {
